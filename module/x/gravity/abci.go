...
func createBatch(ctx sdk.Context, k keeper.Keeper) {
	...
	if !hasUnbatchedTransactions {
		k.Logger(ctx).Info("There are no any pending transactions for "+msg.Denom, "module", "gravity", "action", "auto creation of batches")
		return
	}

	batch, err := k.BuildOutgoingTXBatch(ctx, *tokenContract, keeper.OutgoingTxBatchSize)
	if err != nil {
		k.Logger(ctx).Error("Cannot build outgoing batch: "+msg.Denom, "module", "gravity", "action", "auto creation of batches", "err", err)
		return
	}

	if batch != nil {
		k.Logger(ctx).Info("A batch was created", "module", "gravity", "action", "auto creation of batches")
	} else {
		k.Logger(ctx).Info("There are no any transactions for "+msg.Denom, "module", "gravity", "action", "auto creation of batches")
	}
}
